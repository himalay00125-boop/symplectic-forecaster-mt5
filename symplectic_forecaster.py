...
class MT5TradeExecutor:
    ...
    def close_position(self, position) -> TradeResult:
        ...
        trade = TradeResult(
            True, "CLOSE",
            f"Closed #{position.ticket} {position.volume} lots @ {result.price:.5f} | Ret: {realized_return:.2%}",
    ...
    def on_signal(self, forecast: Dict, symbol: str):
        ...
        if signal.action == "BUY":
            for pos in shorts:
                ...
                if result.success and result.entry_features and self.forecaster and self.forecaster._model:
                    try:
                        self.forecaster._model.learn_one(result.entry_features, result.realized_pnl)
                        result.message += " [Learned Feedback]"
                    except Exception:
                        pass
    ...
    def to_json_dict(self) -> dict:
        with self.lock:
            ...
            if self.engine and hasattr(self.engine, 'executor'):
                log = self.engine.executor.trade_log
                trade_log_data = [
                    {
                        "ticket": t.ticket,
                        "action": t.action,
...
class SymplecticOnlineModel:
    ...
    def _build_river_model(self):
        ...
        self._pa = (
            ...
            linear_model.PARegressor(C=0.005, eps=1e-4, mode=2)
        )
        self._ht = (
            preprocessing.StandardScaler() |
